                 ...
                 const ConfigParameters& params)
{
    ...
    Mirp_lp mirpSolver(pInst, params.getModelParams());
    // mirpSolver.writeModel(params.getOutputDir());
    bool solved = mirpSolver.solve(params.getSolverParams());

    if (solved)
    {
        mirpSolver.writeResultsJSON(params.getOutputDir());
        mirpSolver.writeSolution(params.getOutputDir());
    }
    else
    {
        mirpSolver.writeIis(params.getOutputDir());
    }
}
