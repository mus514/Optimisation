...
private:
    ...
    const std::vector<std::vector<std::vector<GRBVar>>>& m_v;
    ...
    */
    ...
    /**
     * @brief Retrieve the v variables values from the relaxation solution at
     * the current node.
     * @param: constraint type.
     * @return: v variables values at the current node.
    */
    std::vector<std::vector<std::vector<double>>> getvVarsValues(
        const constrsType cstType);
