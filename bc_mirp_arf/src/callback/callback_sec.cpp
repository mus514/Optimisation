    ...
    const constrsType cstType)
{
    std::vector<std::vector<std::vector<double>>> qVal(
        mpInst->getNbVertices(), std::vector<std::vector<double>>(
            mpInst->getNbVertices(), std::vector<double>(mpInst->getT(), 0)
        )
    );

    for (int i = 1; i < mpInst->getNbVertices(); ++i)
    {
        for (int k = 1; k < mpInst->getNbVertices(); ++k)
        {
...
std::vector<std::vector<std::vector<double>>> CallbackSEC::getvVarsValues(
        const constrsType cstType)
{
    std::vector<std::vector<std::vector<double>>> vVal(
        mpInst->getNbVertices(), std::vector<std::vector<double>>(
            mpInst->getNbVertices(), std::vector<double>(mpInst->getT(), 0)
        )
    );
    ...
    for (int i = 1; i < mpInst->getNbVertices(); ++i)
    {
        for (int k = 1; k < mpInst->getNbVertices(); ++k)
        {
            for (int t = 0; t < mpInst->getT(); ++t)
            {
                DCHECK_F(i < static_cast<int>(m_v.size()));
                DCHECK_F(k < static_cast<int>(m_v[i].size()));
                DCHECK_F(t < static_cast<int>(m_v[i][k].size()));
                if (cstType == constrsType::lazy)
                {
                    vVal[i][k][t] = getSolution(m_v[i][k][t]);
                }
                else
                {
                    vVal[i][k][t] = getNodeRel(m_v[i][k][t]);
                }
            }
        }
    }
