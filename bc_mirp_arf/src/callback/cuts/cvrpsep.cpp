...
int CallbackSEC::addCVRPSEPCAP(const constrsType cstType)
{
    ...
    /* get solution: v */
    auto vVal = getvVarsValues(cstType);
    ...
    for (int k = 1; k < n; ++k)
    {
        for (int t = 0; t < mpInst->getT(); ++t)
        {
            ...
            CAPSEP_SeparateCapCuts(n - 1,
                                   demand.data(),
                                   mpInst->getCk(0), // homogeneous fleet
                                   nbEdges,
                                   edgeTail.data(),
                                   edgeHead.data(),
                                   edgeX.data(),
            ...
            for (int c = 0; c < cutsCMP->Size; ++c)
            {
                if (cutsCMP->CPL[c]->CType == CMGR_CT_CAP)
                {
                    ...
                    for (std::size_t i = 0; i < list.size(); ++i)
                    {
                        ...
                        vExpr += m_v[list[i]][k][t];
                        vSum += vVal[list[i]][k][t];
                    }
                    ...
                    for (std::size_t i = 0; i < list.size(); ++i)
                    {
                        ...
                        {
                            addLazy(xExpr <= vExpr - m_v[list[i]][k][t]);
                            ++nbAdded;
                        }
                        else if (xSum - vSum + vVal[list[i]][k][t] >
                                 utils::GRB_EPSILON)
                        {
                            addCut(xExpr <= vExpr - m_v[list[i]][k][t]);
                            ++nbAdded;
                        }
                    }
                }
