 ...
 * File: main.cpp
 *
 * @brief A branch-and-cut algorithm for the Multi-vehicles Inventory-Routing
 * Problem (MIRP) Asymmetric Representatives Formulation (ARF).
 * @author Guilherme O. Chagas (guilherme.o.chagas[a]gmail.com)
 * @date This file was created on January 9, 2021, 01:26 PM
 * @warning I'm sorry for my bad English xD.
 * @acknowledgment Special thanks to Ph.D. Leandro C. Coelho and Ph.D. Mariam
 * Darvish.
                 ...
                 const ConfigParameters& params)
{
    ...
    Mirp_arf bcMirpArf(pInst, params.getModelParams());
    // bcMirpArf.writeModel(params.getOutputDir());
    bool solved = bcMirpArf.solve(params.getSolverParams());

    if (solved)
    {
        bcMirpArf.writeResultsJSON(params.getOutputDir());
        bcMirpArf.writeSolution(params.getOutputDir());
    }
    else
    {
        bcMirpArf.writeIis(params.getOutputDir());
    }
}
